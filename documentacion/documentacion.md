...
-   [run_tests](#run_tests){.function}
-   [update_readme](#update_readme){.function}
...
::: {#run_tests .section}
::: {.attr .function}
[def]{.def} [run_tests]{.name}[([):]{.return-annotation}]{.signature
...
::: {.pdoc-code .codehilite}
     6def run_tests():
     7    """Ejecuta las pruebas de la carpeta actual usando pytest.
     8
     9    Returns:
    10        str: Mensaje de estado de las pruebas con un prefijo seguido
    11             por la fecha y hora actual en formato "YYYY-MM-DD HH:MM:SS".
    12
    13    Raises:
    14        subprocess.CalledProcessError: Si pytest retorna un código distinto de cero.
    15    """
    16    try:
    17        # El metodo strftime sirve para pasar un objeto tipo datetime, date o time a cadena de texto
    18        subprocess.check_call(["pytest", "-q"])
    19        
    20        return f"✅ Tests correctos {datetime.now().strftime('%Y-%m-%d %H:%M:%S')}"
    21    
...
Returns: str: Mensaje de estado de las pruebas con un prefijo seguido
por la fecha y hora actual en formato \"YYYY-MM-DD HH:MM:SS\".

Raises: subprocess.CalledProcessError: Si pytest retorna un código
distinto de cero.
:::
:::
...
::: {.attr .function}
[def]{.def} [update_readme]{.name}[([[status]{.n}[:]{.p}
...
Args: status (str): Mensaje generado por [`run_tests()`](#run_tests)
indicando el estado de las pruebas.
